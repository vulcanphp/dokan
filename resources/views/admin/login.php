...
<div class="w-full h-screen flex items-center justify-center">
    <form method="post" class="w-10/12 sm:w-8/12 md:w-6/12 lg:w-4/12 xl:w-2/12 text-center">
        ...
        <?= csrf() ?>
        <input type="password" required class="w-full px-4 py-2 outline-none text-center focus:outline-xamber-700 rounded-full bg-xbg-700" placeholder="<?= translate('Password') ?>" name="password">
        <button class="bg-xamber-700 hover:bg-xamber-800 px-4 py-2 inline-block mt-4 rounded-full"><?= translate('Login') ?></button>
    </form>
</div>