<div class="flex flex-row flex-wrap justify-center">
    ...
    <div class="w-full md:w-7/12 md:pl-6 mt-6 md:mt-0 text-center md:text-left">
        <div class="bg-xbg-700 rounded-xl px-6 py-5">
            <h2 class="text-3xl mb-2 font-semibold"><?= $product->title ?></h2>
            <p class="mb-6"><?= Currency::$instance->format($product->price, 'text-2xl') ?></p>
            ...
            <p class="mb-6">
                <?= translate('Category') ?>: <a fire href="<?= $product->category->getSlug() ?>" class="text-xamber-700 hover:text-xamber-800"><?= $product->category->title ?></a>
            </p>
            <?php
            if ($product->quantity >= 1) {
                $this->component('components.cart-button', [
                    'product' => $product,
                    'customize' => true,
                    'class' => [
