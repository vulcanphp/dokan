...
<div class="<?= $class['parent'] ?? '' ?> mt-4">
    <?php if ($product->quantity >= 1) : ?>
        <?php if ($customize) : ?>
            <input type="number" x-ref="cartCustomize<?= $product->id ?>" :readonly="hasCart(<?= $product->id ?>)" :value="getQuantity(<?= $product->id ?>)" min="1" max="<?= $product->quantity ?>" class="bg-xbg-700 focus:bg-xbg-600 rounded-l-full outline-none px-4 py-2">
        <?php endif ?>
